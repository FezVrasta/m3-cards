...
| [Lights Overview](#m3-lights-overview-card) | `m3-lights-overview-card` | Raum-für-Raum Lichtübersicht, nach Bereich gruppiert |
...
| [Chip Buttons](#m3-chip-buttons-card) | `m3-chip-buttons-card` | Eine Zeile antippbarer Pillen-Chips für beliebige Entities — wie Bubble Cards Sub-Buttons |
...
- **`mode`**: welche Entities Auto-Discovery meldet und womit jeder Raum
  ...
  beides und bevorzugt den Thermostat-Messwert dort, wo ein Raum keinen
  eigenen Sensor hat (oder wo das Thermostat mehrere physische Geräte
  bündelt). `thermostat_only` überspringt jeden Raum ohne `climate`-Entity.
  Manuelle `rooms` erhalten ein passendes optionales `climate_entity`.

`max_visible` begrenzt, wie viele Räume gleichzeitig sichtbar sind, der Rest
sitzt hinter einem "mehr anzeigen"-Umschalter am Ende des Rasters — nützlich,
sobald Auto-Discovery mehr Räume findet, als bequem hineinpassen.
...
`tap_action` / `hold_action` / `double_tap_action` nehmen die übliche
Lovelace-Action-Config (`toggle`, `more-info`, `perform-action`, `navigate`,
`url`, `none`) plus eine zusätzliche Art — `popup` — die einen Dialog über
der Karte öffnet, gesteuert über `popup.mode`:

- **`default-grid`** (Standard): dieselbe Karte noch einmal, eingegrenzt auf
  den angetippten Raum (bei Auto-Discovery über den Bereich, bei manueller
  Konfiguration über die Entity-Liste). `popup.*` grenzt weiter ein —
  `title`, `sort`, `show_header`, plus die üblichen Filterfelder
  (`exclude_entities`, `exclude_labels`, …), von der äußeren Karte geerbt,
  sofern nicht `inherit_filters: false`.
- **`default-detail`**: Home Assistants eigener More-Info-Dialog für den
  angetippten Raum — ohne eine unserer Karten.
- **`custom`**: eine beliebige Lovelace-Karte, gebaut aus `popup.card`. Jeder
  String-Wert darin darf `[[area_id]]`, `[[device_id]]`, `[[entity_id]]`,
  `[[name]]`, `[[temperature_entity]]`, `[[humidity_entity]]` referenzieren,
  aufgelöst gegen den angetippten Raum, bevor die Karte gebaut wird.

`hold_action` ist standardmäßig `popup`, `tap_action` standardmäßig
`more-info`. Ein explizit gesetztes `tap_action` übernimmt gegenüber
`tile_tap_action: thermostat`, das nur den *Standard*-Tap liefert.
