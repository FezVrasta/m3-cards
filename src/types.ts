...
export interface HaActionConfig {
  action:
    ...
    | "url"
    | "assist"
    // A card-defined popup (e.g. m3-lights-overview re-scoped to one room) rather
    // than HA's own more-info dialog — see shared/actions.ts + popup-card.ts.
    // Ignored by cards that don't implement it, same as any other action.
    | "popup"
    | "none";
  service?: string;
