...
import type { CardHelpers } from "./detail-card";
...
// `CardHelpers` and the `Window.loadCardHelpers` global are declared once,
// in detail-card.ts — re-declaring the global here with a different
// `createCardElement` signature is a TS error, so this module reuses that one.
